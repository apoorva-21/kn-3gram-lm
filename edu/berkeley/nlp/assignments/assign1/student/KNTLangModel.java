 ...
 * 
 * @author apoorva89
 *Done building RAW KNTrigramLanguageModel. Now taking logs of bigram counts and trigram counts
Size of indexer = 495172
...
Distribution for context [in, terms] normalizes correctly, sums to 0.999958064492676
Distribution for context [romanian, independent] normalizes correctly, sums to 0.9996758222619332
Distribution for context [the] normalizes correctly, sums to 1.0000007378523326
Spot checks completed
Reading phrase table from file /home/apoorva89/eclipse-workspace/291_p1/code1/phrasetable.txt.gz {
Line 100000
Line 200000
Line 300000
Line 400000
...
Decoding took 424.838s
BLEU score on test data was BLEU(24.952)

 */
public class KNTLangModel implements NgramLanguageModel
{
...
//	static final double discount = 0.81;// BLEU 24.955
	...
	public KNTLangModel(Iterable<List<String>> sentenceCollection) {
...
//				break;
			//			Size of indexer = 495172
			//			Unique Unigrams = 495170
			//					Unique Bigrams = 8374231
			//					Unique Trigrams = 41701112
...
//					
//					idx_ngram = decodeNgram(encoded_ngram);
//					
...
//								long enc_unigram = encodeNgram();
		...
		System.out.println("Done building RAW KNTrigramLanguageModel. Now taking logs of bigram counts and trigram counts");
		System.out.println("Size of indexer = "+ EnglishWordIndexer.getIndexer().size());

		// ngram2count.computeLogs();
		// ngram2fert.computeLogs();

//		System.out.println("Done taking logs of bigram counts and trigram counts");
		l_unique_bigrams = Math.log(unique_bigrams);
		
//		System.out.println("Max Values : ");
//		System.out.println("ngram2count = "+ ngram2count.getMaxVal());
//		System.out.println("ngram2fert = " +ngram2fert.getMaxVal());
//		System.out.println("mword2fert = " +mword2fert.getMaxVal());
//		System.out.println("mword2fert_count = "+ mword2fert_count.getMaxVal());
//		System.out.println("ntrigramsw1w2 = " +ntrigramsw1w2.getMaxVal());
//		
...
//		System.out.println("mword2fert_count = "+ mword2fert_count.size());
		System.out.println("ntrigramsw1w2 = " +ntrigramsw1w2.size());
		
//		System.exit(1);
//		sentenceCollection = null;
//		System.gc();
...
//					System.out.println("Likelihood of the parameters for sentence till now : " + sent_sum);
//				long encoded_ngram = encodeNgram(idx_ngram);
...
//		System.exit(1);
		...
		if(to - from >= 2)//bigram
		{
			...
			//*********************************REPLACE mword2fert***********************
			// int denominator = mword2fert.get(EnglishWordIndexer.getIndexer().get(sub_bigram[0]));
		...
		{			
			...
			if(denominator > 0) {//only if the denominator is zero then alpha = 1
				//*************************replace implementation of getNTrigrams****************
				long n_trigrams_w1w2 = ntrigramsw1w2.get(encodeNgram(cxt_bg));//number of trigrams starting with w1w2
				TRI_ALPHA_NUM = l_discount3 + Math.log(n_trigrams_w1w2);//WRONG IMPLEMENTATION for d > 1
